...
SELECT f.name, m.firstname AS first_name, m.surname AS sur_name
FROM Bookings AS b
LEFT JOIN Facilities AS f ON b.facid = f.facid
LEFT JOIN Members AS m ON m.memid = b.memid
WHERE f.name LIKE '%Tennis Court%'
GROUP BY first_name, sur_name
ORDER BY first_name, sur_name
LIMIT 0 , 30

Q8:

SELECT bookid, f.facid, f.name, b.memid, m.firstname || " " || m.surname AS full_name, f.membercost, f.guestcost, slots, 
...
SELECT bookid, name, firstname, surname, tot_price
FROM 
...
WHERE mem_tot < 1000 AND guest_tot < 1000;
